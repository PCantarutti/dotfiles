...
while true; do
    ...
    if [ "$wifi_status" = "enabled" ]; then
        current=$(nmcli -t -f ACTIVE,SSID device wifi list | grep "^yes" | cut -d: -f2)
    ...
    chosen=$(echo -e "$options" | rofi -dmenu -p "󰖩  WiFi" -i -normalize-match -theme ~/.config/rofi/wifi-bluetooth.rasi)
    [ -z "$chosen" ] && exit
    ...
    if [[ "$chosen" == *"Desligar WiFi"* ]]; then
        ...
        ssid=$(echo "$chosen" | sed 's/.*󰖩  //' | sed 's/ (.*//' | sed 's/ ✓//')
        password=$(rofi -dmenu -p "🔑 Senha para $ssid" -password -i -normalize-match -theme ~/.config/rofi/wifi-bluetooth.rasi)
        [ -z "$password" ] && exit
