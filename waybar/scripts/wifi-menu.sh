...
while true; do
    ...
    if [[ "$chosen" == *"Desligar WiFi"* ]]; then
        ...
        # Abre rofi de loading
        echo -e "󰑐  Buscando redes..." | rofi -dmenu -p "󰖩  WiFi" -lines 1 -no-custom -theme ~/.config/rofi/wifi-bluetooth.rasi &
        ROFI_PID=$!
        # Escaneia e espera 5 segundos
        nmcli device wifi rescan
        sleep 5
        # Fecha o rofi de loading
